...
import { User as PrismaUser, RequestState, ExpenseRequest as PrismaExpenseRequest, Prisma } from '@prisma/client';
...
const createMockExpenseRequest = (id: number, state: RequestState, requesterId: number, amount = 100): Prisma.ExpenseRequestGetPayload<{include: {requester: true, approver: true, payment: true}}> => ({
  id,
  ...
  attachmentId: 1,
  requesterId,
  requester: { id: requesterId, username: `user${requesterId}`, isAdmin: false },
  approverId: null,
  approver: null,
  paymentId: null,
  payment: null,
...
describe('ExpenseService', () => {
  ...
  describe('Happy Path: DRAFT -> PENDING -> APPROVED -> PAID', () => {
    ...
    const createInput: CreateExpenseRequestInput = { amount: 100, attachmentId: 1 };
    ...
    it('should create a DRAFT expense request and transition to PENDING on SUBMIT', async () => {
      ...
      const pendingExpense = { ...draftExpense, state: 'PENDING' as RequestState }; // Ensure type compatibility
      ...
      const submitted = await service.transitionState(expenseId, { type: 'SUBMIT' });
      expect(submitted.state).toBe('PENDING');
      expect(mockPrismaService.expenseRequest.update).toHaveBeenCalledWith(expect.objectContaining({
        where: { id: expenseId },
        data: { state: 'PENDING' },
      }));
      expect(mockWebhookService.sendWebhook).toHaveBeenCalledWith(expect.objectContaining({
        newState: 'PENDING',
      }));
    });

    it('should transition from PENDING to APPROVED on APPROVE', async () => {
      const pendingExpense = createMockExpenseRequest(expenseId, 'PENDING', mockUser.id);
      const approvedExpense = { ...pendingExpense, state: 'APPROVED' as RequestState, approverId: mockApprover.id, approver: mockApprover, approvedAt: expect.any(Date) }; 

      mockPrismaService.expenseRequest.findUnique.mockResolvedValueOnce(pendingExpense);
      mockPrismaService.expenseRequest.update.mockResolvedValue(approvedExpense);

      const approved = await service.transitionState(expenseId, { type: 'APPROVE', approverId: mockApprover.id }, mockApprover);
      expect(approved.state).toBe('APPROVED');
      expect(approved.approverId).toBe(mockApprover.id);
      expect(approved.approvedAt).toBeInstanceOf(Date);
      expect(mockPrismaService.expenseRequest.update).toHaveBeenCalledWith(expect.objectContaining({
        where: { id: expenseId },
        data: { state: 'APPROVED', approver: { connect: { id: mockApprover.id } }, approvedAt: expect.any(Date) },
      }));
      expect(mockWebhookService.sendWebhook).toHaveBeenCalledWith(expect.objectContaining({
        newState: 'APPROVED',
      }));
    ...
    it('should transition from APPROVED to PAID on PAY', async () => {
      const paymentId = 10;
      const approvedExpense = createMockExpenseRequest(expenseId, 'APPROVED', mockUser.id);
      ...
      const paidExpense = { ...approvedExpense, state: 'PAID' as RequestState, paymentId }; 

      mockPrismaService.expenseRequest.findUnique.mockResolvedValueOnce(approvedExpense);
      mockPrismaService.expenseRequest.update.mockResolvedValue(paidExpense);

      const paid = await service.transitionState(expenseId, { type: 'PAY', paymentId });
  ...
  describe('Invalid Transition: PENDING -> PAID', () => {
    it('should throw BadRequestException for PENDING to PAID transition', async () => {
      const mockUser = createMockUser(1, 'testuser');
      const pendingExpense = createMockExpenseRequest(1, 'PENDING', mockUser.id);
      mockPrismaService.expenseRequest.findUnique.mockResolvedValueOnce(pendingExpense);
   ...
   describe('createDraftExpenseRequest', () => {
    it('should create and return a draft expense request', async () => {
      ...
      const expectedResult = createMockExpenseRequest(1, 'DRAFT', mockUser.id, 50);
  ...
  describe('transitionState general cases', () => {
    it('should throw NotFoundException if expense request does not exist', async () => {
      mockPrismaService.expenseRequest.findUnique.mockResolvedValue(null);
      await expect(service.transitionState(99, { type: 'SUBMIT' })).rejects.toThrow(NotFoundException);
    });
  });
}); 